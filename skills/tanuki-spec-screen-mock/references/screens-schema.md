...
画面は部品の充足だけで確定させず、`references/design-exploration.md`に従って利用者・業務・既存UI・ブランドを調査する。次の5項目は画面ごとに必須で、空欄、値全体が`未定`・`TBD`・`<このモックで決めたい問い>`などの仮値はゲートで弾く。説明文中のタグ風テキストや、入力部品を示す`<select>`・`<input>`など既知のHTML要素名は仮値として扱わない。確定済みの説明文中に「未定義」などの語が含まれるだけでも仮値扱いしない。
