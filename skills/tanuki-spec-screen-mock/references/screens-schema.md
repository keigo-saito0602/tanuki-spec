...
画面は部品の充足だけで確定させず、`references/design-exploration.md`に従って利用者・業務・既存UI・ブランドを調査する。次の5項目は画面ごとに必須で、空欄、`未定`、`TBD`、`<このモックで決めたい問い>`のような仮値はゲートで弾く。確定済みの説明文中に「未定義」などの語が含まれるだけでは仮値扱いしない。
